...
struct SharedThreadData
{
    Container<T> ranges{};
    std::mutex mutex{};
    std::atomic<unsigned> atomicDoneThreadCount{};
    // Ending variables
...
template <typename T>
void quicksort_threaded_aux(SharedThreadData<T> &data)
{
    bool isIdle = false;
    while (data.atomicDoneThreadCount.load() != data.threadCount)
    {
        triple<T> r;
        bool isEmpty = true;
        {
            // Lock to access and use bag of task
            const std::lock_guard<std::mutex> lock(data.mutex);
            isEmpty = data.ranges.empty();
            if(!isEmpty)
            {
                // If not empty, do tasks
                r = data.ranges.TOP();
                data.ranges.POP();
            }
            // Unlock
        ...
        else if(!isIdle) // If nothing was popped and we are not idle
        {
            // Meaning we are supposed to be idle, but we're not.
            // So set label
            isIdle = true;
            ++data.atomicDoneThreadCount;
        }
...
template <typename T>
void quicksort(T *a, unsigned begin, unsigned end, unsigned threadCount)
{
    ...
    {
        threads.emplace_back(std::thread(quicksort_threaded_aux<T>, std::ref(data)));
    }

    // Condition to wait for threads to all finish
    while(data.atomicDoneThreadCount != threadCount)
    {
